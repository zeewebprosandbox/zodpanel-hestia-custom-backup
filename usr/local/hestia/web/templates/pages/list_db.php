		...
		<div class="toolbar-buttons">
			<?php if ($read_only !== "true") { ?>
				...
				<?php if ($_SESSION["DB_SYSTEM"] === "mysql" || $_SESSION["DB_SYSTEM"] === "mysql,pgsql" || $_SESSION["DB_SYSTEM"] === "pgsql,mysql") { ?>
					<a class="button button-secondary <?= tohtml(ipUsed() ? "button-suspended" : "") ?>" href="/open/phpmyadmin/" target="_blank">
						<i class="fas fa-right-to-bracket icon-orange"></i>phpMyAdmin
					</a>
				<?php } ?>
	...
	<div class="units-table js-units-container">
		...
		<?php
			...
			foreach ($data as $key => $value) {
				...
				if (($data[$key]['TYPE'] == 'mysql') && (!empty($_SESSION['DB_PMA_ALIAS']))) $db_admin_link = "https://".$http_host."/".trim($_SESSION['DB_PMA_ALIAS'], "/")."/";
							...
							<?php } ?>
								<?php if ($data[$key]['TYPE'] == 'mysql' && isset($_SESSION['PHPMYADMIN_KEY']) && $_SESSION['PHPMYADMIN_KEY'] != '' && !ipUsed()) { $time = time(); ?>
									<?php
										...
										$hestia_sso_url = "/open/phpmyadmin/?" . http_build_query(["database" => $key]);
									?>
									<li class="units-table-row-action shortcut-enter" data-key-action="href">
										<a
											class="units-table-row-action-link zod-db-visit"
											href="<?= tohtml($hestia_sso_url) ?>"
											title="<?= tohtml( _("Visit phpMyAdmin")) ?>" target="_blank"
										>
										<i class="fas fa-right-to-bracket icon-orange"></i>
										<span><?= tohtml( _("Visit")) ?></span>
									</a>
								</li>
							<?php } ?>
							<li class="units-table-row-action shortcut-enter" data-key-action="href">
