<?php require_once $_SERVER['DOCUMENT_ROOT'] . '/inc/zodpanel_modules.php'; ?>
	...
	: $panel[$user]["CRON_JOBS"] ?>&#13;<?= _("Suspended") ?>: <?= $panel[$user]["SUSPENDED_CRON"] ?>">
 	...
 	"SUSPENDED_CRON"
       	...
       	echo "active";
                ...
                <!-- Modules tab -->
                <?php if (!empty(zodpanel_module_cards($user, $panel[$user] ?? []))) { ?>
                    <li class="main-menu-item">
                        <a class="main-menu-item-link <?php if ($TAB == "MODULES") {
                            echo "active";
                        } ?>" href="/list/modules/" title="<?= _('Enabled package modules') ?>">
                            <p class="main-menu-item-label"><?= _('MODULES') ?><i class="fas fa-layer-group"></i></p>
                            <ul class="main-menu-stats">
                                <li><?= _('Enabled') ?>: <?= count(zodpanel_module_cards($user, $panel[$user] ?? [])) ?></li>
                                <li><?= _('Package') ?>: <?= htmlspecialchars(zodpanel_package_name_for_user($user, $panel[$user] ?? [])) ?></li>
                            </ul>
                        </a>
                    </li>
                <?php } ?>
